...
from vsstubs.cli import app as vsstubs_app
...
def generate_svg(command_args: Sequence[str], output_dir: Path, columns: int) -> Path:
    ...
    if command_args == ["--help"]:
        cmd_title = "vsstubs --help"
        file_name = "vsstubs_help.svg"
    else:
        sub_name = "_".join(arg for arg in command_args if arg != "--help")
        cmd_title = f"vsstubs {' '.join(command_args)}"
        file_name = f"vsstubs_{sub_name}_help.svg"
...
def main(output_dir: Path = Path("assets/svg"), width: int = 110) -> None:
    """Generate SVG help outputs for vsstubs commands.
    ...
    all_commands = discover_commands(vsstubs_app)
